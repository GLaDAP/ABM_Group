...
class Walker(Agent):
    ...
    def move_towards_specified_kind(
        ...
        agent_of_type = [agent for agent in neighbours if isinstance(agent, agent_type)]
        if (filter_func):
    ...
    def __get_closest_agents(self, agent_list : list):
        heap = []
        [heapq.heappush(heap,((abs(self.pos[0] - agent.pos[0]), abs(self.pos[1] - agent.pos[1])), agent)) for agent in agent_list]
